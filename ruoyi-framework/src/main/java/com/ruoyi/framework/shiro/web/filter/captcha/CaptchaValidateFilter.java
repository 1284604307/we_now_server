...
public class CaptchaValidateFilter extends AccessControlFilter
{
            ...
            throws Exception
    {
        ...
        // 只检查 POST
        if (!"post".equals(httpServletRequest.getMethod().toLowerCase())) return true;
        // 获取头信息的平台标识
        if (httpServletRequest.getHeader(AuthType.name)!=null){
            ...
            ShiroUtils.getSession().setAttribute(AuthType.name, AuthType.WEB);
        }
        // 验证码禁用 或携带APP访问头 允许访问
        if (isApp||!captchaEnabled) return true;
